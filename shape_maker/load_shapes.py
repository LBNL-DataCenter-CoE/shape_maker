...
def generate_daily_load_profile(l0, peak_params=[{'start_time': 6 * 60, 'duration': 8*60, 'part_load': 0.6}], noise_random_frac_of_l0=0.1, interval=15, transition_duration=30, transition_smoothness=10.0):
    ...
    random_variation = np.random.uniform(-noise_range, noise_range, size=len(time_minutes))
...
def generate_yearly_load_profile(l0, weekday_peak_values, weekday_dynamic_range,
    ...
    for day in range(num_days):
        ...
        num_peaks = np.random.choice([1, 2, 3], p=peak_freqs)

        # Generate peak parameters for the day
        peak_params = []
        for peak_num in range(num_peaks):
            first_peak_start = np.random.uniform(0, random_first_peak_delay_up_to)
            first_peak_start += possible_time_range[0]
            peak_duration_actual = peak_duration * (1 + np.random.uniform(-1 * peak_duration_variability,
                                                                           peak_duration_variability))
            peak_value = peak_values[peak_num % len(peak_values)]
            peak_load_fraction = np.random.uniform(
        ...
        daily_load_profile = generate_daily_load_profile(
            l0,
            peak_params,
            minimum_noise_random_frac_of_l0 + np.random.uniform(
                0, maximum_noise_random_frac_of_l0 - minimum_noise_random_frac_of_l0),
            interval,
            transition_duration,
            transition_smoothness
        )

        # Add to yearly load profile
...
def correct_l0(l0, average_weekday_duration, average_weekend_duration, average_weekday_increase, average_weekend_increase): # durations in hours
    ...
    corrected_l0 = 24 * 365 * l0 / (\
                       ((24 - average_weekday_duration) + average_weekday_duration * (1+average_weekday_increase)) * count_weekdays + \
                       ((24 - average_weekend_duration) + average_weekend_duration * (1+average_weekend_increase)) * count_weekend_days)
    # print(l0, corrected_l0)
    return corrected_l0


def generate_specific_it_load_profiles(l0, noise_style="low_noise", diurnal_style="flat"):
...
def create_power_components_dataset(yearly_itload_profile, fractional_itload_components, flat_loads_as_percent_of_itcapacity, itload_following_load_efficiencies, file_name=None, save=False):
    ...
    if save:
        power_components.to_csv(os.path.join("power_components_database", file_name))
